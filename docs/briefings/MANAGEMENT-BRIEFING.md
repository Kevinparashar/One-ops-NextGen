...
The runtime principles are documented in `docs/briefings/PROJECT-BRIEFING.md` §2. The single LLM egress is implemented in `src/oneops/llm/gateway.py`. The policy composer lives in `src/oneops/policy/composer.py` (307 lines, 7 policy profiles).
...
Reference package: `src/oneops/use_cases/uc08_fulfillment/` (eleven files: contracts, handlers, core, executor, adapters, agent, NATS dispatcher, judge, catalog search and reranker, priority computation, historical-suggestion logic, text-extract LLM call, and SR-id minting). Contract: `docs/architecture/COMPONENT_SPEC.md` (C1 to C24). Conventions: `docs/architecture/CONVENTIONS.md`. Non-negotiable rules: `docs/briefings/PROJECT-BRIEFING.md` §2. Test pattern: `tests/integration/test_uc08_button_user_journey.py`. Definition-of-done: `docs/planning/production-maturity-plan.md` §D. Verifier: `make pmg-verify` writes `ops/pmg-evidence/REPORT.md`.
...
| V2 agent files' flat top-level lifecycle metadata | The V2 schema uses nested `active_version` and `versions[]`. | Document the V2 schema in `docs/architecture/CONVENTIONS.md`. |

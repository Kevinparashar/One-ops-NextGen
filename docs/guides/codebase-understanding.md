...
> (does not replace) the existing `docs/architecture/ARCHITECTURE.md`, `docs/briefings/PROJECT-BRIEFING.md`,
> `docs/architecture/COMPONENT_SPEC.md`. Read those for the *target* contract; read this for the
...
- This split is the **known structural gap**: production routes all DB access through
  a system-wide **Data Access Layer (DAL)**. See `docs/planning/risk-register.md` and the
...
- Run: `uvicorn` via app factory; a single `docker-compose.yml` (9 base services +
  an opt-in `--profile langfuse` adding 6 Langfuse containers). Local CI gate
  `scripts/ci.sh` (6 stages; smoke+devils are placeholders). No Dockerfile/k8s yet.

...
7. (0.5 h) Manager decision package + PMG demo runbook + run `verify-all.sh` to produce the final evidence report. `docs/manager-decision-package.md` (10 §G questions, recommended answers + cost-of-alternative + downstream gates). `docs/pmg-demo-runbook.md` (script of the meeting).
...
- **Afternoon (4 hr) — Local CI gate (replaces GitHub Actions; no GitHub access at present) + P0-#4 (eval gate, scoped) + manager decision package.**
  ...
  - **Manager decision package (2h):** `docs/manager-decision-package.md` framing each of the 10 §G open questions as: question, recommended answer + rationale, cost of the alternative, downstream gates. Hand-deliverable to manager for binding answers.
...
- Every Day 1 + Day 2 deliverable lands with tests and a `RUNBOOK.md` entry.

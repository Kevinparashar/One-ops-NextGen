...
**Problem.** Top-level README and `docs/architecture/ARCHITECTURE.md` describe three live use cases. Code has two live use cases (Summarization, Knowledge Lookup) plus a conversational fallback. The third, *Action on a Ticket*, is designed and partially scaffolded but has no working handlers.

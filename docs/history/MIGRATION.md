# docs/history/MIGRATION.md — POC-5-MW → Production AI Engine
...
**depends_on**, **excludes**, owner (docs/architecture/ARCHITECTURE.md §4). CRUD APIs; ABAC tags;
...
**Build:** the four-stage funnel (docs/architecture/ARCHITECTURE.md §3) — glossary
...
- Failure-mode tests: a node failure, a hook abort, and a mid-run crash each
  resume correctly from checkpoint (docs/architecture/ARCHITECTURE.md Flow C).
...
docs/architecture/ARCHITECTURE.md §7. Structured, redacted, trace-correlated logs.

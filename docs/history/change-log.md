...
## 2026-06-06 · Consolidate to a single docker-compose + single ops/

- **What**: merged the duplicate `docker-compose.v1.yml` + `ops_v1/` (a full
  port-shifted copy of the base stack plus the Langfuse services) into the main
  `docker-compose.yml` as an opt-in `--profile langfuse` block, and folded the
  Langfuse trace fan-out into `ops/otel/collector.yaml` (best-effort exporter,
  safe when the profile is down). Deleted `docker-compose.v1.yml` + `ops_v1/`.
- **Why**: two parallel stacks were confusing ("v1" read as legacy but was the
  newer Langfuse stack); one file + one `ops/` is the single source of truth.
- **Files**: `docker-compose.yml` (+Langfuse profile, +`LANGFUSE_OTEL_AUTH` on
  collector, +volumes), `ops/otel/collector.yaml` (+filter/langfuse, +otlphttp/
  langfuse, +traces/langfuse pipeline), `docs/runbooks/RUNBOOK.md` + `docs/guides/codebase-understanding.md`
  (single-file commands); removed `docker-compose.v1.yml`, `ops_v1/`.
- **Validation**: `docker compose config` ✅ (9 services) and
  `docker compose --profile langfuse config` ✅ (15 services); `collector.yaml`
  parses. Runtime (`docker compose up`) to be confirmed by operator.
- **Result**: one compose, one `ops/`; default stack unchanged, Langfuse opt-in.
...
- **Validation gates**:
  ...
  - **tenant_id** present as a Langfuse trace attribute (per-tenant filtering);
    secrets via env.
- **Files**: `docs/runbooks/RUNBOOK.md` (§11).
...
- **Files**: `tests/unit/api/test_session_continuity.py`; R-8 in `docs/planning/risk-register.md`.
...
- **What**: Read-only repo-wide scan; produced `docs/guides/codebase-understanding.md`,
  `docs/planning/production-readiness-audit.md`, `docs/planning/refactor-plan.md`, `docs/planning/testing-strategy.md`,
  `docs/planning/risk-register.md`. Captured baseline.
...
REAL CODE (not docs). The scope doc (docs/planning/scheduler-refactor-scope.md) is the plan;

...
| P0-2 | **`AUTHZ_JWT_SECRET` has no default / no fail-fast.** `secret = os.getenv("AUTHZ_JWT_SECRET")` → `None`. | `authz/tokens.py:44` | Missing config yields a cryptic `None`-type failure at verify time instead of a clear boot error. |
| P0-3 | **Stack-trace / exception text leaked to clients.** `detail=f"engine failure: {exc}"`, `detail=f"text-extract failure: {exc}"`. | `api/app.py:2063`, `api/uc08_routes.py:290` | Leaks internal types/messages to HTTP clients; should log internally, return opaque message. |

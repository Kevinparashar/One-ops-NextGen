...
> **How to use this doc.** This is the single document to hand to any AI (or human) who has never seen this codebase before. It contains: what the project is, where to find things, the non-negotiable rules, the current state, how to run it, and what production-grade work means here. After reading this, plus the two cross-referenced docs (`docs/production-maturity-plan.md` and `ARCHITECTURE.md`), they should be able to execute production-grade work without breaking conventions.
...
**Status today:** Demo-mature for UC-1 + UC-3 (routing 96.4%, devil's-play 11/11, smoke 40/40, RAG with calibrated gate, full OTel). Not yet production-mature on five of six axes — see `docs/production-maturity-plan.md`.
...
├── RUNBOOK.md                ← operator commands & dev-env steps
├── MIGRATION.md              ← migration history
├── CLEANUP.md                ← cleanup tasks log
...
├── scripts/                  ← maintenance / smoke / seed scripts
├── src/oneops/               ← application code (see §3.2)
├── tests/                    ← unit + integration tests (73 test files)
└── tools/                    ← seed_supabase, seed_kb_embeddings, gen_proto.sh, migration helpers
```
...
- **`docs/production-maturity-plan.md`** — **READ THIS SECOND.** Gap matrix vs target architecture, 30-item PMG-demonstrable definition of done, P0/P1/P2 roadmap, 2-day cut.
...
- `docs/00_overview.md` / `01_stack_components.md` / `02_uc_pack_structure.md` / `07_starting_steps.md` — onboarding sequence.
- `docs/BEHAVIOR_CORPUS.md` — the eval corpus of expected behaviours.
- `docs/BUILD_STATUS.md` — phase-by-phase build tracker.
- `docs/COMPONENT_SPEC.md` — per-component contract.
...
8. `RUNBOOK.md` updated if there is an operator implication.
9. `MIGRATION.md` updated if there is a migration.
...
Full detail in `docs/production-maturity-plan.md`.
...
1. **This document** (`docs/PROJECT-BRIEFING.md`) — context + rules.
2. **`docs/production-maturity-plan.md`** — what production-mature means here, gap matrix, roadmap.
3. **`ARCHITECTURE.md`** — canonical architecture.
4. **`RUNBOOK.md`** — operator commands.
5. **`docs/COMPONENT_SPEC.md`** — per-component contracts.
6. **`docs/pmg-validation/index.md`** then 01 → 07.
7. **`DECISIONS/ADR-*.md`** — why specific tech choices were made.
...
Captured in `docs/production-maturity-plan.md` §G. The three that block scoping hardest:

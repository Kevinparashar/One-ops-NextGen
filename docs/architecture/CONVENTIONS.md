# docs/architecture/CONVENTIONS.md — coding conventions for `src/oneops/`

These are the conventions the existing platform code is built to. They distil what is already enforced in `src/oneops/` and what `docs/architecture/COMPONENT_SPEC.md` (C1–C24) and `docs/briefings/PROJECT-BRIEFING.md §2` mandate. A new module that violates them will be rejected at review.
...
## 10. Routing & agent definition (`docs/architecture/ARCHITECTURE.md §3`, `§4`)
...
1. `docs/briefings/PROJECT-BRIEFING.md §2` — the 13 non-negotiable rules.
2. `docs/architecture/COMPONENT_SPEC.md` C1–C24 — the per-component contract.

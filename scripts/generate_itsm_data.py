...
Run:  .venv/bin/python scripts/generate_itsm_data.py
...
DATA_DIR = Path(__file__).resolve().parents[1] / "data" / "itsm"
SEED = 20260521
random.seed(SEED)
...
INCIDENT_TPL = {
    ...
    "database": [("Query latency degraded on primary", "Read queries are slow; the primary database CPU is saturated.", "performance", "Core Database"),
                 ("Replication lag on read replica", "The read replica is minutes behind the primary.", "replication", "Core Database"),
                 ("Connection pool exhausted", "The application exhausts the DB connection pool under load.", "connections", "Core Database")],
    "email": [("Outbound mail delayed", "Outgoing email is queued for hours before delivery.", "smtp", "Mail Service"),
              ("Phishing reports spike", "Multiple users report a suspicious email campaign.", "spam", "Mail Service"),
              ("Calendar invites not syncing", "Calendar invitations fail to propagate to attendees.", "calendar", "Mail Service")],
    ...
    "platform": [("Kubernetes pods in CrashLoopBackOff", "Several pods restart continuously after a config change.", "kubernetes", "Container Platform"),
                 ("Autoscaler not adding nodes", "The cluster autoscaler fails to add nodes under load.", "autoscaling", "Container Platform"),
                 ("Ingress controller returns 502", "The ingress controller intermittently returns 502 errors.", "ingress", "Container Platform")],
    ...
    "itsm": [("Ticket notifications not sent", "Assignment notifications are not delivered to agents.", "notifications", "ITSM Platform"),
             ("SLA timers not pausing", "SLA timers do not pause when a ticket is on hold.", "sla", "ITSM Platform"),
             ("Approval step skipped on request", "A request advanced past an approval step incorrectly.", "workflow", "ITSM Platform")],
    "knowledge": [("KB search returns stale results", "Knowledge search surfaces retired articles.", "search", "Knowledge Base"),
                  ("Article images not loading", "Images embedded in KB articles fail to render.", "content", "Knowledge Base"),
                  ("KB feedback widget broken", "The helpful/not-helpful widget does not record votes.", "feedback", "Knowledge Base")],
}
...
def gen_kb(rows: list[dict], users: list[dict], cis: list[dict]) -> list[dict]:
    ...
    for i in range(need):
        t = pick_tenant()
        c = random.choice([k for k in KB_TPL])
...
def backfill(problems, incidents, changes, kb):
    for p in problems:
        pid, t = p["problem_id"], p["tenant_id"]
